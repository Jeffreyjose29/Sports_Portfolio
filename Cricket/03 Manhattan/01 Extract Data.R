...
innings1_manhatten <- bbb %>%
  ...
  mutate(
    OverPeriod = case_when(
      Over >= 0 & Over <= 10 ~ "Powerplay",
      Over >= 11 & Over <= 40 ~ "Middle Overs",
      Over >= 41 & Over <= 50 ~ "Death Overs",
      TRUE ~ "Other"  # Just in case, for safety
    )
  ) %>%
  ggplot(aes(x = Over, y = Runs, fill = OverPeriod)) + 
  geom_bar(stat = "identity", position = "dodge") +
  scale_fill_manual(
    values = c("Powerplay" = "green", 
               "Middle Overs" = "yellow", 
               "Death Overs" = "red", 
               "Other" = "grey")  # Fallback if anything unexpected appears
  ) +
  labs(x = "Over", y = "Runs") +
  theme_minimal() +
  theme(
...
gg_grob_manhattan <- ggplotGrob(innings1_manhatten)
gg_image_manhattan_innings1 <- image_graph(width = 1900, height = 400, res = 98)
grid::grid.draw(gg_grob_manhattan)
dev.off()



innings2_manhatten <- bbb %>%
  filter(innings == 2) %>%
  group_by(Over) %>%
  summarise(
    Runs = sum(runs_off_bat, na.rm = TRUE) + 
...
gg_grob_manhattan <- ggplotGrob(innings2_manhatten)
